...
if TYPE_CHECKING:
  from cat_mjlab.envs import CaTManagerBasedRlEnv
...
class CaT:
  ...
  def __init__(self, tau: float = 0.95, min_p: float = 0.0):
    self.running_maxes: dict[
      str, torch.Tensor
    ] = {}  # Polyak average of max constraint violation
  ...
  def add(self, name: str, constraint: torch.Tensor, max_p: float = 0.1):
    ...
    if mask.any():
      ...
      probs[mask] = self.min_p + torch.clamp(normalized[mask], 0.0, 1.0) * (
        max_p - self.min_p
      )

    self.probs[name] = probs
    self.max_p[name] = torch.full(
      (constraint.shape[1],), max_p, dtype=torch.float, device=self._device
    )
  ...
  def get_max_p(self) -> torch.Tensor:
    return (
      torch.cat(list(self.max_p.values()))
      if self.max_p
      else torch.tensor([], device=self._device)
    )
  ...
  def get_vals(self) -> list[float]:
    return [
      100.0 * probs.max(1).values.gt(0.0).float().mean().item()
...
class ConstraintTermCfg(ManagerTermBaseCfg):
  max_p: float = 1.0
  """Maximum termination probability."""
...
class ConstraintManager(ManagerBase):
  ...
  def __init__(
    ...
    for term_name in self._term_names:
      self._episode_sums[term_name] = torch.zeros(
        self.num_envs, dtype=torch.float, device=self._device
      )
  ...
  def reset(self, env_ids: torch.Tensor | slice | None = None) -> dict[str, float]:
    ...
    for key in self._episode_sums.keys():
      ...
      # Set to one in all episodes where length was 0
      episode_lengths[episode_lengths == 0] = 1
  ...
  def _prepare_terms(self):
    cfg_items = (
      self.cfg.items() if isinstance(self.cfg, dict) else self.cfg.__dict__.items()
    )
